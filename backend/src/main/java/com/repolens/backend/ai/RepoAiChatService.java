    ...
    public Optional<String> answerQuestion(RepositoryProject repository, List<RepositoryFile> files, Analysis analysis, String question) {
        ...
        try {
            ...
            String userPrompt = semanticChunks.isEmpty()
                    ? promptBuilder.structuredRepositoryQuestionPrompt(repository, files, analysis, question)
                    : promptBuilder.structuredRagRepositoryQuestionPrompt(repository, analysis, question, semanticChunks);
            ...
            log.debug("RepoPilot AI RAG chat failed; using rule-based fallback.", ex);
            return Optional.empty();
        }
    }
}